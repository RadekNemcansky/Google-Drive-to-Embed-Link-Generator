...
function generateLink(e) {
  ...
  const confirmLink = gLink.value.includes("https://drive.google.com/drive/");
  console.log(gLink.value);
  ...
  if (confirmLink == true) {
    const getDownloadLink = gLink.value
      ...
      .replace("/view?usp=share_link", "/view?usp=share_link");
    ...
    function copyText(target) {
      if (target.value == "") {
        ...
        navigator.clipboard.writeText(target.value).then(() => {
          alert("Link has been coppied to clickboard");
        });
      }
    }
    ...
    const getVideoLink = gLink.value.replace("?usp=share_link", "");
